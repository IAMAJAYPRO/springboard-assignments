...
class Tester {
    ...
    public static List<Deque<Patient>> splitQueue(Deque<Patient> patientsQueue) {
        ...
        Deque<Patient> less = new ArrayDeque<Patient>(), more = new ArrayDeque<Patient>();
		for (Patient p: patientsQueue){
			(p.getAge()<60? less: more).add(p);
		}
		ArrayList<Deque<Patient>> arr=new ArrayList<Deque<Patient>>();
		arr.add(more);
		arr.add(less);
		return arr;
